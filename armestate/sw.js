/* ArmEstate service worker.

   The app ships a standalone PWA manifest, so an agent who installs it expects it to OPEN
   without signal — in a stairwell, in a basement flat, outside a building on a cheap Android.
   Without a precached shell, a relaunch offline gives a browser error page, and the agent who
   just showed a flat loses the viewing record that makes the fee claim defensible.
...
const CACHE = 'armestate-shell-v12';
   /* v12: apple-design polish on the glass layer. touch-action removes the tap-recognition
      ...
      prefers-contrast is answered. A cached v11 shell would keep the compounded translucency. */
...
/* THE DISPLAY FONTS ARE PART OF THE SHELL, not optional assets. An app that opens offline in
   ...
   THE PATHS POINT INTO /armtrade/. THREE products share one typeface and one set of files rather
   than shipping three copies — one download, one cache entry, and a browser that already has
   ArmTrade or ArmVoski installed pays nothing at all.

   KNOWN LIMIT, stated rather than hidden: this worker's SCOPE is /armestate/, so it does not
   ...
   scope to '/' or a copy of the two woff2 files under /armestate/vendor/. That is a deployment
...
self.addEventListener('fetch', e => {
  ...
  const isShell = req.mode === 'navigate' || url.pathname.endsWith('/armestate/') ||
